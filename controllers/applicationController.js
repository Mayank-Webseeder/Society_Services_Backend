...
exports.applyToJob = async (req, res) => {
  try {
    ...
    if (job.status === "Completed") {
      return res
        .status(400)
        .json({ msg: "Cannot apply. This job is already completed." });
    }
...
exports.showInterestInJob = async (req, res) => {
  try {
    ...
    if (job.status === "Completed") {
      return res
        .status(400)
        .json({ msg: "Cannot show interest. This job is already completed." });
    }
    ...
    res
      .status(500)
      .json({ msg: "Failed to show interest", error: err.message });
  }
};
...
exports.getJobApplicants = async (req, res) => {
  try {
    ...
    if (applications.length === 0 && process.env.NODE_ENV !== "production") {
      ...
      const sample = await Application.find()
        .limit(5)
        .select("job vendor status applicationType");
      console.warn(sample);
    }
    ...
    res
      .status(500)
      .json({ msg: "Failed to get applicants", error: err.message });
  }
};
...
exports.debugListApplicationsForJob = async (req, res) => {
  ...
  try {
    ...
    }).select("job vendor applicationType status createdAt");
    return res.json({ count: apps.length, apps });
  } catch (err) {
    return res
      .status(500)
      .json({ msg: "Error fetching debug data", error: err.message });
  }
};
...
exports.debugListOrphanApplications = async (req, res) => {
  ...
  try {
    ...
    const existingJobs = await Job.find({
      _id: { $in: distinctJobIds },
    }).select("_id");
    const existingSet = new Set(existingJobs.map((j) => j._id.toString()));
    const orphanIds = distinctJobIds.filter(
      (id) => !existingSet.has(id.toString()),
    );

    const orphans = await Application.find({ job: { $in: orphanIds } }).select(
      "job vendor applicationType status createdAt",
    );
    ...
    return res
      .status(500)
      .json({
        msg: "Error fetching orphaned applications",
        error: err.message,
      });
  }
};
...
exports.approveApplication = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Error approving application", error: err.message });
  }
};
...
exports.getVendorApplicationType = async (req, res) => {
  try {
    ...
    if (!application) {
      return res
        .status(404)
        .json({ msg: "No application or interest found for this vendor" });
    }
    ...
    res
      .status(500)
      .json({
        msg: "Failed to fetch vendor application type",
        error: err.message,
      });
  }
};
...
exports.getApplicantCount = async (req, res) => {
  try {
    ...
    const quotationApplications = applications.filter(
      (a) => a.isQuotation,
    ).length;
    ...
    res
      .status(500)
      .json({ msg: "Error fetching applicant count", error: err.message });
  }
};
...
exports.rejectApplication = async (req, res) => {
  try {
    ...
    if (application.job.status !== "New") {
      return res
        .status(400)
        .json({ msg: "Cannot reject applications after job is completed." });
    }
    ...
    res
      .status(500)
      .json({ msg: "Error rejecting application", error: err.message });
  }
};
