...
exports.getVendorDashboard = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Failed to fetch dashboard", error: err.message });
  }
};
...
exports.updateVendorProfile = async (req, res) => {
  try {
    ...
    // ✅ HANDLE LOCATION SAFELY (INSIDE FUNCTION)
    if (req.body.location) {
      const { latitude, longitude, formattedAddress } = req.body.location;
...
exports.getFeedbacks = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Failed to fetch feedbacks", error: err.message });
  }
};
