...
exports.signupVendor = async (req, res) => {
  try {
    const { name, contactNumber, password, location } = req.body;

    // ✅ location validation
    if (!location || location.latitude == null || location.longitude == null) {
      return res.status(400).json({
        msg: "Location (latitude & longitude) is required",
      });
    }
    ...
    const newVendor = new Vendor({
      ...
      location: {
        type: "Point",
        coordinates: [
          location.longitude, // ⚠️ lng first
          location.latitude, // lat second
        ],
      },
    });
...
exports.createVendorProfile = async (req, res) => {
  try {
    ...
    if (req.idProofFile && req.idProofFile.url) {
      vendor.idProof = req.idProofFile.url;
    }

    // 🔢 Convert numbers explicitly. Use null when not provided to avoid storing 0 as accidental default
    const latitude =
      req.body["location.GeoLocation.latitude"] !== undefined &&
      req.body["location.GeoLocation.latitude"] !== ""
        ? Number(req.body["location.GeoLocation.latitude"])
        : null;

    const longitude =
      req.body["location.GeoLocation.longitude"] !== undefined &&
      req.body["location.GeoLocation.longitude"] !== ""
    ...
    if (req.body.idProofFile) {
      vendor.idProof = req.body.idProofFile;
    }
    ...
    if (!vendor.idProof) {
      return res
        ...
        .json({
          success: false,
          msg: "ID proof file is required to complete profile",
        });
    }
