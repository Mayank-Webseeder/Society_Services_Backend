...
exports.allSociety = async (req, res) => {
  try {
    const data = await Society.find().select(
      "_id societyname address location residentsCount email isApproved status createdAt updatedAt"
    );

    res.status(200).json(data);
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: "error in getting society data" });
  }
};
...
exports.getPendingSocieties = async (req, res) => {
  try {
    const pending = await Society.find({ status: "Pending" }, "societyname");
    res.status(200).json({
...
exports.getApprovedSocieties = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Error fetching approved", error: err.message });
  }
};
